...
class GoogleWifi:
  ...
  async def get_api_token(self):
    ...
    return True
    oauth_url = "https://oauthaccountmanager.googleapis.com/v1/issuetoken"
    ...
    _LOGGER.warning(f"Getting API token from {oauth_url} using {payload} ")
    
    response = await self.post_api(oauth_url, headers, payload)
    
    _LOGGER.warning(f"Got a response {response}")
